// Server-only filesystem browsing for the project-root folder picker.
//
// Local tool only (GIT_TOOL_ENABLED gated). Lets the UI walk the host's folders
// to pick a project root, since a browser can't read absolute paths. Every path
// is path.resolve()'d — never string-concatenated from client input — and only
// directory listings (name + isGitRepo hint) are ever returned, never file
// contents. There is no base restriction: folder layout is per-machine and the
// caller already has the developer's own filesystem access.
...
/** Does `dir` contain a `.git` entry (dir or file)? Cheap repo hint for the picker. */
  ...
  const entries: DirEntry[] = await Promise.all(
    dirs
      ...
      .map(async (d): Promise<DirEntry> => {
        ...
        return { name: d.name, path: full, isRepo: await hasGit(full) };
      }),
  );
