...
//
//   POST { path?: string, marker?: string }
//        → { path, parent, entries[], isDriveList, markerHere? }
//
...
//
// Returns directory listings only — never file contents. Enabled by default
// because DevBox is a localhost dev tool; set FS_BROWSE_ENABLED=0 to switch
// folder listing off entirely.
...
export async function POST(req: NextRequest) {
  ...
  try {
    return NextResponse.json(await browse(target, marker));
  } catch (e) {
